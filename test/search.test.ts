...
describe("Search", () => {
  it("Simple search", async () => {
    let result = await cl.search({
      query: "PARAMTYP",
      scopes: { product: "ADaMIG v1.1" },
    });
  ...
  it("Search with multiple scopes", async () => {
    let result = await cl.search({
      query: "PARAM",
      scopes: { product: "ADaMIG v1.1", core: "Required" },
    });
  ...
  it("Resolve a hit in ADaM", async () => {
    let result = await cl.search({
      query: "PARAM",
      scopes: { product: "ADaMIG v1.1", type: "Analysis Variable" },
    });
  ...
  it("Resolve a hit in SDTM", async () => {
    let result = await cl.search({
      query: "LBTESTCD",
      scopes: { product: "SDTMIG v3.2", type: "SDTM Dataset Variable" },
    });
    let variable = await result.hits[0].resolve(cl);
  ...
  it("Resolve a hit in CDASH", async () => {
    let result = await cl.search({
      query: "RACE",
      scopes: { product: "CDASHIG v1.1", type: "Data Collection Field" },
    });
