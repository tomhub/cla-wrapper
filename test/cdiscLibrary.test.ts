...
describe("CDISC Library", () => {
  ...
  it("Get an itemGroup", async () => {
    ...
    const sdtmDomainCsv = await cl.getItemGroup("ae", "sdtmig33", {
      format: "csv",
    });
  ...
  it("Get itemGroups", async () => {
    ...
    expect(
      Object.values(adamDss).map((ds) => ds.toSimpleObject()),
    ).toMatchSnapshot();
  });
  ...
  it("Product details", async () => {
    const pdShort = await cl.getProductDetails({
      type: "short",
      format: "csv",
    });
    let pdLong = await cl.getProductDetails({ type: "long", format: "json" });
    pdLong = pdLong.filter((pd) =>
      [
        "adam-2-1",
        "adamig-1-1",
        "sdtmig-3-3",
