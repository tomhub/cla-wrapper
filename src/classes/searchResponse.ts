...
export class SearchResponseHit {
  ...
  constructor(rawHit: { [name: string]: any }) {
    ...
    if (
      ...
      ["Class", "SDTM Dataset", "CDASH Domain", "Data Structure"].includes(
        rawType,
      )
    ) {
      this.type = "ItemGroup";
    ...
    if (Array.isArray(rawHit.links)) {
      ...
      rawHit.links.forEach((link) => {
        if (
          link.linkName === "parentProduct" &&
          /\/mdr\/[^/]+\/.+/.test(link.href)
        ) {
          if (
            link.href.startsWith("/mdr/ct/") ||
            link.href.startsWith("/mdr/adam/")
          ) {
          ...
          link.linkName.toLowerCase() === "parentdatastructure" &&
          /\/mdr\/.*\/datastructures\/.+/.test(link.href)
        ) {
          this.ids.dataStructureId = link.href.replace(/.*\/(.*)$/, "$1");
        } else if (
          (link.linkName === "parentVariableSet" &&
            /\/mdr\/.*\/varset\/.+/.test(link.href)) ||
          (link.linkName === "parentDomain" &&
            /\/mdr\/.*\/domains\/.+/.test(link.href)) ||
          (link.linkName === "parentDataset" &&
            /\/mdr\/.*\/datasets\/.+/.test(link.href))
        ) {
          this.ids.itemGroupId = link.href.replace(/.*\/(.*)$/, "$1");
        } else if (
          link.linkName === "parentClass" &&
          /\/mdr\/.*\/classes\/.+/.test(link.href)
        ) {
          this.ids.dataClassId = link.href.replace(/.*\/(.*)$/, "$1");
        } else if (
          link.linkName === "parentCodelist" &&
          /\/mdr\/.*\/codelists\/.+/.test(link.href)
        ) {
  ...
  async resolve(
    cl: CdiscLibrary,
  ): Promise<Variable | Field | Domain | DataStructure | Dataset> {
    if (this.type === "Item") {
      const itemGroup = (await cl.getItemGroup(
        this.ids.itemGroupId ?? this.ids.dataStructureId,
        this.ids.productId,
      )) as Domain | DataStructure | Dataset;
      return await itemGroup.getItem(this.rawHit.name);
    }
    if (this.type === "ItemGroup") {
      return (await cl.getItemGroup(this.rawHit.name, this.ids.productId)) as
        | Domain
        | DataStructure
        | Dataset;
    }
