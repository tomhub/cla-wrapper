...
import toSimpleObject from "../utils/toSimpleObject.js";
import {
  MatchingOptions,
  GetItemGroupOptions,
...
export class CoreObject {
  ...
  async apiRequest(
    endpoint: string,
    { headers, returnRaw = false, noCache = false }: ApiRequestOptions = {},
  ): Promise<any> {
    // Default options
    try {
...
export class CdiscLibrary {
  ...
  constructor({
    ...
    useNciSiteForCt,
    nciSiteUrl,
  }: CdiscLibraryParameters = {}) {
    this.coreObject = new CoreObject({
      apiKey,
      baseUrl,
      cache,
      traffic,
  ...
  async checkConnection(): Promise<object> {
    ...
    try {
      response = await this.coreObject.apiRequest("/mdr/lastupdated", {
        returnRaw: true,
        noCache: true,
      });
    ...
    if (dataRaw.hasOwnProperty("_links")) {
      Object.keys(dataRaw._links).forEach((pcId) => {
        if (pcId !== "self") {
          ...
          productClasses[pcId] = new ProductClass({
  ...
  async getProductList(
    ...
    pcList.forEach((pcId) => {
      result = result.concat(
        this.productClasses[pcId].getProductList() as string[],
      );
    });
  ...
  async getItemGroup(
    ...
    for (const productClass of Object.values(this.productClasses)) {
      result = await productClass.getItemGroup(
        name,
        productAlias,
        defaultedOptions,
      );
  ...
  async getProductDetails({
    type = "short",
    format = "json",
  }: GetItemGroupsOptions = {}): Promise<ProductDetails[]> {
  ...
  async getCtFromNciSite(
    ...
    if (!this.coreObject.useNciSiteForCt) {
      throw Error(
        "getCtFromNciSite function requires useNciSiteForCt set to true.",
      );
    }
    ...
    await Promise.all(
      pathList.map(async (path) => {
        ...
        // Keep only odm.xml
        const aTags = rawHtml.matchAll(
          /<a\s*href=".*?">(?:.*?Terminology|CDISC Glossary)\s*\d{4}-\d{2}-\d{2}.odm.xml\s*<\/a>/g,
        );
        for (const tag of aTags) {
          const name: string = tag[0].replace(
            /<a\s*href=".*?">(.*?)\s*(?:Terminology)?\s*\d{4}-\d{2}-\d{2}.odm.xml\s*<\/a>/,
            "$1",
          );
          const version: string = tag[0].replace(
            /<a\s*href=".*?">.*?(?:Terminology)?\s*(\d{4}-\d{2}-\d{2}).odm.xml\s*<\/a>/,
            "$1",
          );
          let idName: string;
    ...
    if (
      this.productClasses?.terminology?.productGroups?.packages?.products !==
      undefined
    ) {
  ...
  getTrafficStats(
    ...
    if (type === "all") {
      traffic =
        this.coreObject.traffic.incoming + this.coreObject.traffic.outgoing;
   ...
   */
  async getProductIdByAlias(
    alias: string,
  ): Promise<
    | { productClassId: string; productGroupId: string; productId: string }
    | undefined
  > {
    let result;
  ...
  async getScope(name: string): Promise<string[]> {
    const rawResult = await this.coreObject.apiRequest(
      "/mdr/search/scopes/" + name,
    );
...
export class ProductClass extends BasicFunctions {
    ...
    if (pcRaw.hasOwnProperty("_links")) {
      Object.keys(pcRaw._links).forEach((pgId) => {
        if (pgId !== "self") {
          ...
          productGroups[pgId] = new ProductGroup({
            coreObject: this.coreObject,
          });
  ...
  getProductList(format: "json" | "csv" = "json"): object | string {
    ...
    pgList.forEach((pgId) => {
      result = result.concat(
        this.getProductGroups()[pgId].getProductList() as string[],
      );
    });
    return convertToFormat(result, format);
  }
  ...
  async getItemGroup(
    ...
    for (const productGroup of Object.values(this.productGroups)) {
      result = await productGroup.getItemGroup(
        name,
        productAlias,
        defaultedOptions,
      );
  ...
  getProductIdByAlias(
    alias: string,
  ): { productGroupId: string; productId: string } | undefined {
    let result;
...
export class ProductGroup extends BasicFunctions {
  ...
  getProductList(format: "json" | "csv" = "json"): string[] | object {
    let result: string[] = [];
    if (this.products) {
      result = Object.keys(this.getProducts()).map(
        (pId) => this.products[pId].id,
      );
    } else {
      result = [];
    }
  ...
  getProductIdByAlias(alias: string): { productId: string } | undefined {
    ...
    if (this.products) {
      ...
      productId = productList.find(
        (id) => alias.toLowerCase() === id.toLowerCase(),
      );
      // Remove - and .
      if (!productId) {
        productId = productList.find(
      ...
      // Search by substring
      if (!productId) {
        productId = productList.find((id) =>
          id
            .toLowerCase()
            .replace(/[-. ]/g, "")
            .includes(alias.toLowerCase().replace(/[-. ]/g, "")),
        );
      }
    }
    if (productId) {
  ...
  async getFullProduct(
    ...
    if (idObj !== undefined) {
      ...
      if (loadBasicInfo) {
        ...
        const productRaw = await this.coreObject.apiRequest(
          this.products[id].href,
        );
  ...
  async getItemGroup(
    ...
    if (idObj) {
      return await this.products[idObj.productId].getItemGroup(
        name,
        defaultedOptions,
      );
    }
...
export class Product extends BasicFunctions {
  ...
  parseResponse(pRaw: any, nciSiteResponse = false): void {
    if (!nciSiteResponse) {
      ...
      const rawXml = parser.parse(pRaw);
  ...
  async getItemGroup(
      ...
      if (this.datasetType.toLowerCase() === "datastructures") {
        ...
        if (dsRaw?._links?.parentClass) {
          ...
          if (this?.dataClasses?.hasOwnProperty(dataClass.id)) {
            ...
            this.dataClasses[dataClass.id][
              this.datasetType as "datasets" | "domains"
            ][result.id] = result;
          } else {
            // Otherwise create a new data class
            dataClass[this.datasetType as "datasets" | "domains"] = {
              [result.id]: result,
            };
    ...
    if (defaultedOptions.format === undefined) {
      ...
      return result.getFormattedItems(defaultedOptions.format) as
        | ItemGroupType
        | string;
    }
  }
   ...
   */
  async getCodeListList(
    options: GetItemGroupsOptions = { type: "long" },
  ): Promise<
    Array<{ conceptId: string; preferredTerm: string; href?: string }>
  > {
    const result: Array<{
      conceptId: string;
    ...
    if (!this.codelists) {
      ...
      if (
        clRaw.hasOwnProperty("_links") &&
        clRaw._links.hasOwnProperty("codelists")
      ) {
        const codelists: { [name: string]: CodeList } = {};
  ...
  async getCodeList(
    ...
    if (
      (ct === undefined && !this.fullyLoaded) ||
      (ct && ct.terms.length < 1)
    ) {
  ...
  removeContent(): void {
    if (this.datasetType === "codelists") {
      ...
      if (
        this.dataClasses !== undefined &&
        Object.keys(this.dataClasses).length > 0
      ) {
        this.dataClasses = {};
      }
      if (
...
export class DataStructure extends BasicFunctions {
  ...
  getItems(): Items {
    ...
    if (this.analysisVariableSets) {
      Object.values(this.analysisVariableSets).forEach(
        (analysisVariableSet) => {
  ...
  findMatchingItems(name: string, options: MatchingOptions): ItemType[] {
    ...
    if (this.analysisVariableSets) {
      Object.values(this.analysisVariableSets).some((analysisVariableSet) => {
        const matches = analysisVariableSet.findMatchingItems(
          name,
          defaultedOptions,
        );
        if (matches.length > 0) {
  ...
  getFormattedItems(
    format: "json" | "csv",
    addItemGroupId = false,
  ): string | ItemType[] {
    let result: object[] = [];
    if (this.analysisVariableSets) {
      Object.values(this.analysisVariableSets).forEach(
        (analysisVariableSet: AnalysisVariableSet) => {
          result = result.concat(
            analysisVariableSet.getFormattedItems("json", addItemGroupId, {
              dataStructure: this.id,
            }) as ItemType[],
          );
        },
      );
...
export class DataClass extends BasicFunctions {
  ...
  getItemGroups(): ItemGroups {
    let result = {};
    if (
      typeof this.domains === "object" &&
      Object.keys(this.domains).length > 0
    ) {
      result = { ...result, ...this.domains };
    } else if (
      typeof this.datasets === "object" &&
  ...
  findMatchingItems(name: string, options: MatchingOptions): ItemType[] {
    ...
    if (
      this.classVariables &&
      !(defaultedOptions.firstOnly && result.length > 0)
    ) {
    ...
    if (itemRaw.hasOwnProperty(this.itemType)) {
      itemRaw[this.itemType].forEach((itemRaw: any) => {
        ...
        if (this.itemType === "fields") {
          ...
          ["analysisVariables", "datasetVariables"].includes(this.itemType)
        ) {
          item = new Variable({
            name: itemRaw.name,
            href,
  ...
  async getVersions(): Promise<string[]> {
    ...
    if (this.href && this.conceptId) {
      ...
      if (
        ...
        response._links &&
        Array.isArray(response._links.versions)
      ) {
        response._links.versions.forEach((version: any) => {
          result.push(
            version.href.replace(/.*\/mdr\/ct\/packages\/(.*?)\/.*/, "$1"),
          );
        });
      }
    }
...
abstract class Item extends BasicFunctions {
  ...
  constructor({
    ...
    if (id) {
      ...
      if (/\/(?:datastructures|datasets|domains)\//.test(href)) {
        this.id = href.replace(
          /.*\/(?:datastructures|datasets|domains)\/(.*?)\/.*\/(.*)$/,
          "$1.$2",
        );
      } else {
        this.id = href.replace(/.*\/(.*)$/, "$1");
  ...
  parseItemResponse(itemRaw: any): void {
    ...
    if (itemRaw.hasOwnProperty("_links")) {
      if (
        itemRaw._links.codelist &&
        Array.isArray(itemRaw._links.codelist) &&
        itemRaw._links.codelist.length > 0 &&
        itemRaw._links.codelist[0].href
      ) {
        this.codelistHref = itemRaw._links.codelist[0].href;
        this.codelist = itemRaw._links.codelist[0].href.replace(
          /.*\/(\S+)/,
          "$1",
        );
    ...
    if (this.codelistHref) {
      ...
      if (rootCodeListRaw?._links?.versions) {
        ...
        if (ctVer) {
          ...
          href =
            rootCodeListRaw._links.versions[
              rootCodeListRaw._links.versions.length - 1
            ].href;
        }
        if (href) {
...
export class Field extends Item {
  ...
  constructor({
    ...
    simpleDatatype,
    mappingInstructions,
    sdtmigDatasetMappingTargetsHref,
    codelist,
    codelistHref,
    type,
    href,
  ...
  parseResponse(fRaw: any): void {
    ...
    if (fRaw.hasOwnProperty("_links")) {
      if (fRaw?._links?.sdtmigDatasetMappingTargets?.href) {
        this.sdtmigDatasetMappingTargetsHref =
          fRaw._links.sdtmigDatasetMappingTargets.href;
      }
    }
  }
}
